...
export const Testpage = () => {
  ...
  // for mobile menu dropdown items
  const mobDropdownRef = React.createRef();
  const openmobDropdownPopover = () => {
    createPopper(mobDropdownRef.current, popoverDropdownRef.current, {
      placement: "bottom-start",
    });
    setDropdownPopoverShow(true);
  };
  const closemobDropdownPopover = () => {
    setDropdownPopoverShow(false);
  };
      ...
      <div className="navbar bg-[#ea7f23]">
        ...
        <div className="md:hidden flex bg-blue-300 items-center">
          <div className=" ml-auto mr-0 ">
            <button
              className=" font-bold uppercase text-sm inline-flex px-6 py-2 outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150"
              type="button"
              ref={mobDropdownRef}
              onClick={() => {
                dropdownPopoverShow
                  ? closemobDropdownPopover()
                  : openmobDropdownPopover();
              }}
            >
              <img
                src="/static/icons/menu_50px.png"
